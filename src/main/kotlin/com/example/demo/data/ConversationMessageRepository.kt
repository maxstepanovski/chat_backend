...
interface ConversationMessageRepository : JpaRepository<ConversationMessageEntity, Long> {
    ...
    @Query(
            "SELECT * FROM public.conversation_message WHERE conversation_id = ?1 ORDER BY message_id DESC LIMIT 1",
            nativeQuery = true
    )
    fun findLatestByConversationId(conversationId: Long): ConversationMessageEntity
}